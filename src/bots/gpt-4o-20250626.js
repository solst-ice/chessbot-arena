...
function isPassedPawn(board, row, col, color) {
  const direction = color === COLORS.WHITE ? 1 : -1;
  const endCol = color === COLORS.WHITE ? 0 : 7;
  // Note: adjusted for horizontal board orientation
  // We consider columns instead of rows for pawn advancement.

  for (let c = col + direction; color === COLORS.WHITE ? c <= 7 : c >= 0; c += direction) {
    // Check adjacent ranks (rows)
...
function isIsolatedPawn(board, row, col, color) {
  // Check adjacent ranks for pawns of same color
...
function isDoubledPawn(board, row, col, color) {
  // Check same rank for another pawn of same color (horizontal orientation)
  for (let c = 0; c < 8; c++) {
    if (c !== col) {
  ...
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      ...
      if (piece && piece.type === PIECE_TYPES.ROOK) {
        let ownPawns = 0; let oppPawns = 0;
        for (let c = 0; c < 8; c++) {
          const p = board[row][c];
          if (p && p.type === PIECE_TYPES.PAWN) {
            if (p.color === piece.color) ownPawns++; else oppPawns++; }
        }
        if (piece.color === botColor) {
          if (!ownPawns && !oppPawns) score += 20; else if (!ownPawns) score += 10;
        } else {
          if (!ownPawns && !oppPawns) score -= 20; else if (!oppPawns) score -= 10;
        }
      }
    }
  }
  ...
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      ...
      if (piece && piece.type === PIECE_TYPES.BISHOP) {
        if (piece.color === COLORS.WHITE) whiteBishops++; else blackBishops++; }
    }
  }
  if (botColor === COLORS.WHITE) {
    if (whiteBishops >= 2) return 15; if (blackBishops >= 2) return -15;
  } else {
    if (blackBishops >= 2) return 15; if (whiteBishops >= 2) return -15;
  }
  return 0;
}
...
function isKnightOnOutpost(board, pos, color) {
  ...
  for (let dRow = -1; dRow <= 1; dRow += 2) {
    const row = pos.row + dRow; const col = pos.col - direction;
    if (row >= 0 && row < 8 && col >= 0 && col < 8) {
      const piece = board[row][col];
      if (piece && piece.type === PIECE_TYPES.PAWN && piece.color === color) { protectedByPawn = true; break; }
    }
  }
  ...
  for (let dRow = -1; dRow <= 1; dRow += 2) {
    ...
    if (row >= 0 && row < 8 && col >= 0 && col < 8) {
      const piece = board[row][col];
      if (piece && piece.type === PIECE_TYPES.PAWN && piece.color === opponentColor) { return false; }
    }
  }
  return true;
}
    ...
    for (let col = 0; col < 8; col++) {
      ...
      for (let toRow = 0; toRow < 8; toRow++) {
        for (let toCol = 0; toCol < 8; toCol++) {
          const from = { row, col }; const to = { row: toRow, col: toCol };
          if (isValidMove(gameState.board, from, to, gameState)) {
            const testBoard = makeTestMove(gameState.board, from, to);
            if (!isInCheck(testBoard, color)) { moves.push({ from, to }); }
          }
        }
      }
    }
...
// Create new game state after a test move
...
function orderMoves(moves, gameState) {
  const scoredMoves = moves.map(move => {
    ...
    if (targetPiece) {
      score += 100 * PIECE_VALUES[targetPiece.type] - PIECE_VALUES[fromPiece.type];
    }
    if (fromPiece.type === PIECE_TYPES.PAWN) {
      const promotionCol = fromPiece.color === COLORS.WHITE ? 7 : 0;
      if (move.to.col === promotionCol) score += 900;
    }
    if (CENTER_SQUARES.some(s => s.row === move.to.row && s.col === move.to.col)) score += 30;
    if (fromPiece.type === PIECE_TYPES.KING && Math.abs(move.to.row - move.from.row) === 2) score += 50;
    return { move, score };
  });
...
function getOpeningBookMove(gameState, botColor) {
  ...
  if (book && book[moveCount]) {
    ...
    const validMoves = moves.filter(m => isValidMove(gameState.board, m.from, m.to, gameState));
    if (validMoves.length) return validMoves[Math.floor(Math.random() * validMoves.length)];
  }
  return null;
}

// Position-specific evaluation via piece-square tables
function getPositionValue(piece, row, col, isEndgame) {
  const table = PIECE_SQUARE_TABLES[piece.type];
  if (!table) return 0;
  if (piece.type === PIECE_TYPES.PAWN) return table[piece.color][row][col] / 10;
  if (piece.type === PIECE_TYPES.KING) return (isEndgame ? table.endgame : table.middlegame)[row][col] / 10;
