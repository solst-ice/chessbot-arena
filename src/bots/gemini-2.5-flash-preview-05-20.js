/**
 * Gemini 2.5 Flash Preview 05-20 Chess Bot
 * 
 * An advanced chess bot with a focus on tactical aggression and king safety.
 * It implements the following prioritized evaluation system:
...
const TIME_LIMIT = 2800; // 2.8 seconds
  ...
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      ...
      if (piece && piece.type === PIECE_TYPES.ROOK) {
        ...
        if (piece.color === botColor) {
          if (!ownPawns && !oppPawns) score += 20;
          else if (!ownPawns) score += 10;
        } else {
          if (!ownPawns && !oppPawns) score -= 20;
          else if (!oppPawns) score -= 10;
        }
      }
    }
  }
...
function evaluateBishopPair(board, botColor) {
  ...
  if (botColor === COLORS.WHITE) {
    if (whiteBishops >= 2) return 15;
    if (blackBishops >= 2) return -15;
  } else {
    if (blackBishops >= 2) return 15;
    if (whiteBishops >= 2) return -15;
  }
